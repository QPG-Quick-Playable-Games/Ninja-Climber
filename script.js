...
btnHow.addEventListener('click', ()=>{ howto.classList.remove('howto-hidden'); });
btnHub.addEventListener('click', ()=>{ /* anchor handles navigation */ });
document.getElementById('how-close').addEventListener('click', ()=>{ howto.classList.add('howto-hidden'); });
...
function resetToMenu(){
  running = false; gameOver = false; menu.classList.remove('menu-hidden'); menu.classList.add('menu-visible');
}
...
function loop(now){
  ...
  if(running && !gameOver){
    ...
    for(let i=spikes.length-1;i>=0;i--){ const s = spikes[i]; if(circleDist(px,py,s.x,s.y) < COLLIDE_SPIKE){
      ...
      spikes.splice(i,1);
      if(lives <= 0){ gameOver = true; running = false; msgEl.textContent = 'Game Over — Click to return to menu'; msgEl.classList.add('game-over'); msgEl.style.opacity = 1; if(score > highScore){ highScore = score; localStorage.setItem('nc_high', highScore); msgEl.textContent = 'New High Score! Click to return to menu'; } }
      break;
    } }
